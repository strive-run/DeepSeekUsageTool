...
enum DeepSeekLogoPath {
    private static let viewBoxSize: CGFloat = 50
    ...
    static func makePath(in rect: CGRect) -> CGPath {
        let basePath = SVGPathParser(pathData).parse()
        let scale = min(rect.width, rect.height) / viewBoxSize
        let scaledWidth = viewBoxSize * scale
        let scaledHeight = viewBoxSize * scale
    ...
    func parse() -> CGPath {
        ...
        while index < tokens.count {
            ...
            case "M":
                ...
                path.move(to: point)
                while hasNumberAhead, let nextPoint = readPoint() {
                    path.addLine(to: nextPoint)
                }
            case "C":
                while hasNumberAhead {
                    guard let first = readPoint(), let second = readPoint(), let end = readPoint() else {
                        return path
                    }
                    path.addCurve(to: end, control1: first, control2: second)
                }
            case "L":
                while hasNumberAhead, let point = readPoint() {
